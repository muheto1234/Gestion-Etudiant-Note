...
from .models import Etudiant, Cour, Note
...
class acceuil2(ListView):
    model = Note
    context_object_name = 'form'
    template_name = 'acceuil2.html'
    paginate_by = 10
...
def create_note(request):
    if request.method == 'POST':
        ...
        etudiant, _ = Etudiant.objects.get_or_create(NomEtudiant=etudiant_name,PrenomEtudiant=etudiant_prenom,Matricule=etudiant_matricule)
        cours, _ = Cour.objects.get_or_create(NomCours=cours_name)

        note = Note(etudiant=etudiant, cours=cours, note=note_value)
        note.save()
...
def modifier_etudiant(request, etudiant_id):
    etudiant = Etudiant.objects.get(CodeEtudiant=etudiant_id)
